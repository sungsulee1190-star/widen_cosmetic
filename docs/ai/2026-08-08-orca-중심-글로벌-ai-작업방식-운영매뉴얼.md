...
- `설정 스크립트 추가`: 작업 지시가 아니라 worktree 생성 시 실행할 서버·초기화 명령 설정
...
7. Orca의 `설정 스크립트 추가`에는 작업 프롬프트가 아닌 서버 실행 명령만 등록한다. 예: `python -m http.server 4173`.
8. 처음에는 한 작업을 수동 signal로 끝까지 통과시킨 뒤 자동화를 켠다.
